...
module.exports = grammar({
  ...
  rules: {
    ...
    _header: ($) => choice($.header_require, $.header_import),

    header_require: ($) =>
      seq("@require:", optional($.whitespace), field("require", $.pkgname)),
    header_import: ($) =>
      seq("@import:", optional($.whitespace), field("import", $.pkgname)),
    ...
    let_rec_stmt: ($) => seq("let-rec", sep1("and", $.let_rec_inner)),
    ...
    type_stmt: ($) => seq("type", sep1("and", $.type_inner)),
    ...
    _arg: ($) =>
      choice(field("arg", $._pattern), seq("?:", field("optarg", $._pattern))),
    ...
    sig_stmt: ($) => seq("sig", repeat($._sig_inner), "end"),

    struct_stmt: ($) => seq("struct", repeat($._statement), "end"),

    _sig_inner: ($) =>
      choice($.sig_type_stmt, $.sig_val_stmt, $.sig_direct_stmt),
    ...
    type_prod: ($) =>
      prec.left(PREC.typeprod, seq($._type_expr, "*", $._type_expr)),
    ...
    _pat_cons: ($) =>
      choice(seq($._pattern, "::", $.pat_as), $.pat_variant, $._pattern),
    ...
    match_arm: ($) => seq($.pat_as, optional($.match_guard), "->", $._expr),
    ...
    lambda: ($) =>
      prec.right(
        PREC.lambda,
        seq("fun", repeat1(field("arg", $._pattern)), "->", $._expr)
      ),
    ...
    record_member: ($) => seq($._unary, "#", $.identifier),
    ...
    list: ($) => choice(seq("[", sep(";", $._expr), optional(";"), "]")),

    tuple: ($) => seq("(", sep2(",", $._expr), ")"),

    expr_with_mod: ($) =>
      seq(alias($._module_prefix, $.module_name), ".(", $._expr, ")"),

    modvar: ($) =>
      seq(alias($._module_prefix, $.module_name), ".", $.identifier),
